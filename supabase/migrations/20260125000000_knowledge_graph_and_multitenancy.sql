...
-- 1. Multi-tenancy for workflows and questions
ALTER TABLE workflows ADD COLUMN IF NOT EXISTS profile_id TEXT REFERENCES profile(id);
ALTER TABLE questions ADD COLUMN IF NOT EXISTS profile_id TEXT REFERENCES profile(id);

-- Index for multi-tenant queries
CREATE INDEX IF NOT EXISTS idx_workflows_profile ON workflows(profile_id);
CREATE INDEX IF NOT EXISTS idx_questions_profile ON questions(profile_id);
